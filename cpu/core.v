...
module tinyqv_core #(parameter NUM_REGS=16, parameter REG_ADDR_BITS=4) (
    ...
    reg [17:16] mip_reg;
    wire [NUM_INTERRUPTS:0] mip = {timer_interrupt, interrupt_req[NUM_INTERRUPTS-1:2], mip_reg};
    reg [NUM_INTERRUPTS:0] mie;
    ...
    always @(posedge clk) begin
        if (!rstn) mcause <= 0;
            ...
            if (is_interrupt) begin
                mcause[5] <= 1;
                casez (mip[NUM_INTERRUPTS:0] & mie[NUM_INTERRUPTS:0])
                    11'b1??????????: mcause[4:0] <= 5'h07;
                    11'b0?????????1: mcause[4:0] <= 5'h10;
                    11'b0????????10: mcause[4:0] <= 5'h11;
                    11'b0???????100: mcause[4:0] <= 5'h12;
                    11'b0??????1000: mcause[4:0] <= 5'h13;
                    11'b0?????10000: mcause[4:0] <= 5'h14;
                    11'b0????100000: mcause[4:0] <= 5'h15;
                    11'b0???1000000: mcause[4:0] <= 5'h16;
                    11'b0??10000000: mcause[4:0] <= 5'h17;
                    11'b0?100000000: mcause[4:0] <= 5'h18;
                    11'b01000000000: mcause[4:0] <= 5'h19;
    ...
    always @(posedge clk) begin
        if (!rstn || is_double_fault) begin
            ...
            if (imm_lo == 12'h304) begin
                if (is_csr_write) mie[NUM_INTERRUPTS] <= data_rs1[3];
                else if (is_csr_set) mie[NUM_INTERRUPTS] <= mie[NUM_INTERRUPTS] | data_rs1[3];
                else if (is_csr_clear) mie[NUM_INTERRUPTS] <= mie[NUM_INTERRUPTS] & ~data_rs1[3];
            end
            ...
            if (imm_lo == 12'h304) begin
                if (is_csr_write) mie[9:8] <= data_rs1[1:0];
                else if (is_csr_set) mie[9:8] <= mie[9:8] | data_rs1[1:0];
                else if (is_csr_clear) mie[9:8] <= mie[9:8] & ~data_rs1[1:0];
            end
        end
    end
    ...
    always @(*) begin
        case (imm_lo) 
            ...
            // mie
            12'h304: csr_read = (counter == 1) ? {mie[NUM_INTERRUPTS], 3'b000} :
                                (counter == 4) ? mie[3:0] :
                                (counter == 5) ? mie[7:4] : 
                                (counter == 6) ? {2'b00, mie[9:8]} : 4'b0000;
            ...
            // mip
            12'h344: csr_read = (counter == 1) ? {mip[NUM_INTERRUPTS], 3'b000} :
                                (counter == 4) ? mip[3:0] :
                                (counter == 5) ? mip[7:4] : 
                                (counter == 6) ? {2'b00, mip[9:8]} : 4'b0000;
